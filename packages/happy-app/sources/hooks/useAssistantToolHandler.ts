...
import { machineSpawnNewSession, sessionAllow, sessionDeny } from "@/sync/ops";
...
async function executeTool(tool: string, args: Record<string, any>): Promise<string> {
    switch (tool) {
        ...
        case 'list_sessions': return listSessions(args as { activeOnly?: boolean });
...
function listSessions(args: { activeOnly?: boolean }): string {
    const state = storage.getState();

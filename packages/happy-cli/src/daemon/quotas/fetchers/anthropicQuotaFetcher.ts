...
export function createAnthropicQuotaFetcher(params?: Readonly<{
    ...
    return {
        ...
        fetch: async ({ token, now, signal }): Promise<QuotaSnapshotV1 | null> => {
            // Anthropic usage API requires OAuth tokens (sk-ant-oat*), not plain API keys (sk-ant-api*)
            if (!token.startsWith('sk-ant-oat') && !token.startsWith('ey')) return null;
            ...
            const meters: QuotaSnapshotV1['meters'] = [];

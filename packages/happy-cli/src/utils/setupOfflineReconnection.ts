...
export interface SetupOfflineReconnectionOptions {
    ...
    /** Task ID to link session to (passed through to session creation on reconnect) */
    taskId?: string | null;
    /**
...
export function setupOfflineReconnection(opts: SetupOfflineReconnectionOptions): SetupOfflineReconnectionResult {
    const { api, sessionTag, metadata, state, response, taskId, onSessionSwap } = opts;
    ...
    if (!response) {
        ...
        reconnectionHandle = startOfflineReconnection<ApiSessionClient>({
            ...
            onReconnected: async () => {
                const resp = await api.getOrCreateSession({ tag: sessionTag, metadata, state, taskId });
